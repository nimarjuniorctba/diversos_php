						...
						{if $qte_registros>0}					  
							{foreach $array_fornecedor item=$fornecedor}						 
								<tr>
								  ...
								  <td>{$fornecedor.dt_cadastro}</td>
								  <td>{$fornecedor.status}</td>
								  <td>
									  <a href="{$pagina->base}produtos-fornecedores/alterar/{$fornecedor.idurl}"><img src="{$pagina->base}imagens/icones/editar.png" style="width:20px;height:20px;margin-right:5px;cursor: pointer;"></a>
									  <img src="{$pagina->base}imagens/icones/lixo.png" style="width:20px;height:20px;margin-right:10px;cursor: pointer;" class="btn-excluir" data-id="{$fornecedor.idurl}">
								  </td>
								</tr>
							{/foreach}
						{else}	
