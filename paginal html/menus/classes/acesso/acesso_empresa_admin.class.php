...
class acesso_empresa_admin extends empresa_admin{
 ...
 //          var_dump($objeto);exit;
    ...
    public function verificaEmailEdicao($pdo, $email, $ignorarId=0){
        $sql = "SELECT ema_id FROM mod_empresa_admin
                 WHERE ema_email=:email AND ema_status<>:excluido";
        if((int)$ignorarId > 0){
            $sql .= " AND ema_id<>:id";
        }
        $sql .= " LIMIT 1";
    ...
    public function alteraStatus($pdo, $id, $status){
        ...
        $sql = "UPDATE mod_empresa_admin SET ema_status=:status
                 WHERE ema_id=:id AND ema_status<>:excluido";
        $p_sql = $pdo->prepare($sql);
