...
//            var_dump($objeto);exit;            
            ...
            if ($verifica == false) {
                ...
                $p_sql->bindValue(":admin", $objeto->getAdmin(), PDO::PARAM_INT);
                $p_sql->execute();
                if ($p_sql->rowCount() > 0) {
...
//echo $tipo;exit;
    ...
    public function Deletar($pdo, $valor = '') {
        try {
            ...
            $sql = " UPDATE mod_empresas SET emp_status=:status";
            $sql .= " WHERE emp_id=:id ";
    ...
    public function alteraStatus($pdo, $id, $status) {
        ...
        $sql = "UPDATE mod_empresas SET emp_status=:status
                 WHERE emp_id=:id AND emp_status<>:excluido";
        $p_sql = $pdo->prepare($sql);
    ...
    public function verificaDocumentoEdicao($pdo, $tipo, $documento, $ignorarId=0) {
        $campo = $tipo === 'j' ? 'emp_pj_cnpj' : 'emp_pf_cpf';
        $sql = "SELECT emp_id FROM mod_empresas
                 WHERE {$campo}=:documento AND emp_status<>:excluido";
        if ((int)$ignorarId > 0) {
            $sql .= " AND emp_id<>:id";
        }
        $sql .= " LIMIT 1";
        $p_sql = $pdo->prepare($sql);
        $p_sql->bindValue(":documento", $documento, PDO::PARAM_STR);
        $p_sql->bindValue(":excluido", 'e', PDO::PARAM_STR);
        if ((int)$ignorarId > 0) {
            $p_sql->bindValue(":id", $ignorarId, PDO::PARAM_INT);
        }
        $p_sql->execute();
