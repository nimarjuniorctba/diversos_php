...
public function atualizaDados( $pdo, clientes $objeto){
    ...
    public function listarDados($pdo, $tipo='',$valor=''){
        try{
           ...
           switch ($tipo){
               case 'empresa':
                   $sql     .=      " WHERE cli_status<>:excluido AND emp_id_fk=:empresa ORDER BY cli_id DESC"; 
                   $p_sql    =      $pdo->prepare($sql);
    ...
    /**
     * Altera entre ativo e inativo sem permitir que um registro excluído
     * seja reativado acidentalmente.
     */
    public function alteraStatus($pdo, $id, $empresa, $status){
        try{
            if(!in_array($status, array('a', 'i'), true)){
                return false;
            }

            $sql = "UPDATE mod_clientes
                       SET cli_status=:status
                     WHERE cli_id=:id
                       AND emp_id_fk=:empresa
                       AND cli_status<>:excluido";

            $p_sql = $pdo->prepare($sql);
            $p_sql->bindValue(":status", $status, PDO::PARAM_STR);
            $p_sql->bindValue(":id", $id, PDO::PARAM_INT);
            $p_sql->bindValue(":empresa", $empresa, PDO::PARAM_INT);
            $p_sql->bindValue(":excluido", 'e', PDO::PARAM_STR);
            $p_sql->execute();
    ...
    /**
     * Exclusão lógica limitada à empresa do usuário autenticado.
     */
    public function deletarPorEmpresa($pdo, $id, $empresa){
        try{
            $sql = "UPDATE mod_clientes
                       SET cli_status=:status
                     WHERE cli_id=:id
    ...
    /**
     * Retorna o cliente somente quando pertence à empresa informada.
     */
    public function retornaDadosEmpresa($pdo, $id, $empresa){
        $cliente = $this->retornaDados($pdo, 'id', $id);

        if(is_object($cliente) && (int)$cliente->getEmpresa() === (int)$empresa){
            return $cliente;
        }
